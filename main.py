        ...
        for item in line:
            ...
            if item != "":
                if n == self.columns.title:
                    ...
                    MessageTitle = MessageTitle.replace("Disc 1", "").replace("Disc 2", "").replace("Disc 3", "").replace("Disc 4", "").replace("Side C", "").replace("SideD", "")
...
async def on_message(message):
    ...
    if check_reply(message):
        ...
        for console in ConsoleList:
            if ((' ' + console.name.replace('.csv', '').lower()+ ' ') in message.content.lower()) or (message.content.lower().startswith(console.name.replace('.csv', '').lower())):
